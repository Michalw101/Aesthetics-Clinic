            ...
            <div className="hidden md:flex items-center gap-6 text-sm font-medium tracking-wide uppercase">
              ...
              {isAdmin && (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 text-brand-gold font-bold",
                ...
                >
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {order.items.map((item: any, i: number) => (
                      <div
                        key={i}
                        className="w-12 h-12 rounded-lg bg-brand-beige overflow-hidden flex-shrink-0"
                      >
                        <img
                          src={item.img}
...
function AdminDashboard({
  ...
  const [tab, setTab] = useState<
    "appointments" | "orders" | "products" | "blog" | "reviews" | "treatments"
  >("appointments");
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="p-6">
                      <select
                        value={order.status}
                        onChange={(e) =>
                          onUpdateOrder(order.id, e.target.value)
                        }
                    ...
                    <td className="p-6">
                      <button className="p-2 hover:bg-brand-beige rounded-full">
                        <Info size={18} />
                      </button>
                    </td>
                  </tr>
...
function BookingPage({
  ...
  // שליפת שעות פנויות מהשרת בכל פעם שהתאריך משתנה
  useEffect(() => {
    if (!bookingData.date) {
      setAvailableTimes([]);
      return;
  ...
  // ולידציה ושליחת הטופס לבקאנד
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    // 1. ולידציה בסיסית של שדות ריקים
    if (
    ...
    // 2. ולידציה למספר טלפון ישראלי (תומך ב-05X-XXXXXXX או 05XXXXXXXX)
    ...
    setIsSubmitting(true);
    try {
      // שליחת הנתונים במבנה המדויק ששרת ה-FastAPI מצפה לקבל
      ...
      onNavigate("profile"); // ניווט חזרה לפרופיל האישי
    } catch (error: any) {
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <input
              type="date"
              min={new Date().toISOString().split("T")[0]} // מונע בחירת תאריכים בעבר
