...
import {
  ...
  LogIn, UserPlus, LogOut, CheckCircle2, Package, Users, Trash2, Edit3, Shield
} from 'lucide-react';
...
export default function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      text: "שלום! אני אסיסטנטית Aesthetics Clinic. איך אוכל לעזור לך היום בטיפוח העור או במידע על הטיפולים שלנו?",
    },
  ]);
  ...
  useEffect(() => {
    if (profile) {
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: `שלום ${profile.name}! ברוכה השבה לקליניקה. איך אוכל לעזור לך היום?`,
        },
      ]);
    }
  }, [profile?.uid]);
  ...
  useEffect(() => {
    if (authError) {
      toast.error(authError, { position: "top-center", duration: 4000 });
    }
  }, [authError]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "מצטערת, חלה שגיאה בחיבור. אנא נסי שוב מאוחר יותר.",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const handlePlaceOrder = async () => {
    ...
    const checkoutParams = {
      ...
      cardTokenOrRaw: "tok_visa_12345_encrypted_simulation" 
    };
    ...
    <div className="min-h-screen bg-brand-beige text-brand-dark font-sans selection:bg-brand-gold/30" dir="rtl">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-md border-b border-brand-gold/10 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-8">
            <h1 className="text-2xl serif font-semibold tracking-wide text-brand-gold cursor-pointer" onClick={() => navigateTo("home")}>
              Aesthetics
            </h1>
            ...
            <div className="hidden md:flex items-center gap-6 text-sm font-medium tracking-wide uppercase">
              ...
              {isAdmin && (
                <button onClick={() => navigateTo('admin')} className={cn("flex items-center gap-1 text-brand-gold font-bold transition-colors", currentPage === 'admin' && "underline underline-offset-4")}>
                  <Shield size={14} /> אזור ניהול
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <button onClick={() => navigateTo('profile')} className={cn("text-xs font-bold hover:text-brand-gold transition-colors hidden sm:inline", currentPage === 'profile' ? "text-brand-gold" : "text-brand-dark/60")}>
                  שלום, {profile?.name || "משתמשת"}
                </button>
                <button onClick={logout} className="p-2 rounded-full hover:bg-brand-beige text-brand-dark transition-colors" title="התנתקות">
                  <LogOut size={18} />
                </button>
              </div>
            ) : (
              <button onClick={() => setShowAuthForm(true)} className="flex items-center gap-2 text-sm font-bold text-brand-gold hover:text-brand-gold/80 transition-colors">
                <LogIn size={18} />
                <span className="hidden sm:inline">התחברות</span>
              </button>
            )}
            <button onClick={() => navigateTo('booking')} className="bg-brand-gold text-white px-6 py-2 rounded-full text-sm font-semibold hover:bg-brand-gold/90 transition-all shadow-md">
              הזמיני תור
            </button>
          </div>
        </div>
      ...
      <div className="pt-20">
        <AnimatePresence mode="wait">
          ...
          {currentPage === 'treatments' && <TreatmentsPage key="treatments" treatments={treatments} reviews={reviews} user={user} profile={profile} onAddReview={addReview} />}
          {currentPage === 'store' && <StorePage key="store" onAddToCart={addToCart} cartCount={cart.length} onCheckout={handlePlaceOrder} catalogProducts={products} />}
          {currentPage === 'booking' && <BookingPage key="booking" profile={profile} onNavigate={navigateTo} />}
      ...
      <button
        onClick={() => { setIsAssistantOpen(true); setHasNewMessage(false); }}
      ...
      >
        ...
        {hasNewMessage && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full animate-bounce">1</span>}
      </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-brand-gold/10 flex items-center justify-between bg-brand-beige/30">
                ...
                <button onClick={() => setIsCartOpen(false)} className="p-2 hover:bg-brand-gold/10 rounded-full transition-colors"><X size={20} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {cart.map((item, idx) => {
                  ...
                  return (
                    <div key={item.id || idx} className="flex items-center gap-4 p-3 bg-brand-beige/20 rounded-2xl border border-brand-gold/5">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs bg-brand-gold/10 text-brand-gold px-2 py-0.5 rounded-full font-bold">כמות: {item.quantity || 1}</span>
                          <p className="text-brand-gold font-bold text-sm">₪{(itemPrice * (item.quantity || 1)).toFixed(0)}</p>
                        </div>
                      </div>
                      <button onClick={() => removeFromCart(idx)} className="text-red-400 hover:text-red-600 p-2"><Trash2 size={16} /></button>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="p-6 bg-brand-beige/30 border-t border-brand-gold/10 space-y-4">
                ...
                <button
                  onClick={() => { setIsCartOpen(false); handlePlaceOrder(); }}
      ...
      <AnimatePresence>
        {isAssistantOpen && (
          <div className="fixed inset-0 z-50 flex items-end justify-start p-4 pb-6 md:px-8 md:pt-8 md:pb-8 pointer-events-none">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col pointer-events-auto border border-brand-gold/20 h-[70vh] md:h-[500px]">
              <div className="bg-brand-gold p-6 text-white flex items-center justify-between">
                ...
                <button onClick={() => setIsAssistantOpen(false)} className="hover:bg-white/10 p-2 rounded-full transition-colors"><X size={20} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-brand-beige/30">
                {messages.map((msg, idx) => (
                  <div key={idx} className={cn("flex", msg.role === 'user' ? "justify-start" : "justify-end")}>
                    <div className={cn("max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed", msg.role === 'user' ? "bg-brand-gold text-white rounded-tr-none" : "bg-white text-brand-dark border border-brand-gold/10 rounded-tl-none shadow-sm")}>
              ...
              <div className="p-4 border-t border-brand-gold/10 bg-white">
                <div className="relative">
                  <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === "Enter" && handleSend()} placeholder="איך אוכל לעזור?" className="w-full bg-brand-beige/50 border border-brand-gold/20 rounded-full py-3 pr-4 pl-12 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 text-sm" />
                  <button onClick={handleSend} disabled={!input.trim() || isLoading} className="absolute left-1.5 top-1/2 -translate-y-1/2 w-8 h-8 bg-brand-gold text-white rounded-full flex items-center justify-center hover:bg-brand-gold/90 disabled:opacity-50 transition-all">
...
function HomePage({ onNavigate }: { onNavigate: (page: Page) => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-20">
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1570172619380-41067eeec9d2?auto=format&fit=crop&q=80&w=1920" alt="Clinic Interior" className="w-full h-full object-cover opacity-40" referrerPolicy="no-referrer" />
        ...
        <div className="relative z-10 text-center space-y-8 px-6 max-w-4xl">
          <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
            <h2 className="text-6xl md:text-8xl serif font-light tracking-tight text-brand-dark leading-tight">יופי שמתחיל <br /> <span className="text-brand-gold italic">מבפנים</span></h2>
          </motion.div>
          <motion.p initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="text-lg md:text-xl text-brand-dark/70 max-w-2xl mx-auto leading-relaxed">
            קליניקת Aesthetics מציעה את הטיפולים המתקדמים ביותר בעולם האסתטיקה, בשילוב טכנולוגיה חדשנית ויחס אישי לכל מטופלת.
          </motion.p>
          <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="flex flex-wrap justify-center gap-4">
            <button onClick={() => onNavigate('booking')} className="bg-brand-dark text-white px-10 py-4 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-brand-gold transition-all shadow-xl">הזמיני תור עכשיו</button>
            <button onClick={() => onNavigate('treatments')} className="bg-white border border-brand-gold text-brand-gold px-10 py-4 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-brand-gold hover:text-white transition-all shadow-lg">צפי בטיפולים</button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12">
        <div className="text-center space-y-4">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto shadow-md text-brand-gold"><Star size={32} /></div>
        ...
        <div className="text-center space-y-4">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto shadow-md text-brand-gold"><Sparkles size={32} /></div>
        ...
        <div className="text-center space-y-4">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto shadow-md text-brand-gold"><User size={32} /></div>
...
function TreatmentsPage({ treatments: clinicTreatments, reviews, user, profile, onAddReview }: { treatments: any[]; reviews: any[]; user: any; profile: any; onAddReview: (d: any) => void; }) {
  ...
  const getTreatmentStats = (treatmentName: string) => {
    ...
    return { rating: (sum / treatmentReviews.length).toFixed(1), count: treatmentReviews.length };
  };

  const handleReviewSubmit = () => {
    if (!user) { toast.error("אנא התחברי כדי להשאיר ביקורת"); return; }
    onAddReview({ treatmentId: selectedTreatment.name, userId: user.uid, userName: profile?.name || "משתמשת", rating: reviewForm.rating, comment: reviewForm.comment });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto px-6 py-12 space-y-12">
              ...
              <div className="space-y-2 text-center md:text-right flex-1">
                <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                  <div className="flex text-brand-gold">
                    {[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < Math.floor(Number(stats.rating)) ? "currentColor" : "none"} />)}
                  </div>
                ...
                <div className="pt-4 flex flex-wrap justify-center md:justify-start gap-4">
                  <button onClick={() => setSelectedTreatment(t)} className="text-xs font-bold uppercase tracking-widest text-brand-gold hover:underline">הוסיפי ביקורת</button>
                  {reviews.filter((r) => r.treatmentId === t.name && r.status === "approved").length > 0 && (
                    <button className="text-xs font-bold uppercase tracking-widest text-brand-dark/40 hover:text-brand-dark transition-colors">צפי בכל הביקורות</button>
                  )}
                </div>
              </div>
      ...
      <AnimatePresence>
        {selectedTreatment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setReviewForm({ ...reviewForm, rating: star })} className={cn("transition-transform hover:scale-110", reviewForm.rating >= star ? "text-brand-gold" : "text-gray-200")}>
                      <Star size={32} fill={reviewForm.rating >= star ? "currentColor" : "none"} />
                    </button>
                  ))}
                </div>
                <textarea placeholder="ספרי לנו על החוויה שלך..." value={reviewForm.comment} onChange={(e) => setReviewForm({ ...reviewForm, comment: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-2xl p-4 h-32 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 resize-none" />
              </div>
              <div className="flex gap-4">
                <button onClick={handleReviewSubmit} className="flex-1 bg-brand-gold text-white py-3 rounded-xl font-bold uppercase tracking-widest hover:bg-brand-gold/90 transition-all">שלחי ביקורת</button>
                <button onClick={() => setSelectedTreatment(null)} className="flex-1 bg-brand-beige text-brand-dark py-3 rounded-xl font-bold uppercase tracking-widest hover:bg-brand-beige/80 transition-all">ביטול</button>
              </div>
            </motion.div>
          </div>
        )}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto px-6 py-12 space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        {cartCount > 0 && (
          <button onClick={onCheckout} className="bg-brand-gold text-white px-8 py-3 rounded-full font-bold flex items-center gap-3 shadow-lg hover:scale-105 transition-transform">
            <ShoppingBag size={20} /> בצעי הזמנה ({cartCount})
          </button>
        )}
      </div>
          ...
          <div key={p.id || idx} className="bg-white rounded-3xl overflow-hidden border border-brand-gold/10 group">
            ...
            <div className="p-6 space-y-2">
              ...
              <div className="flex justify-between items-center pt-4">
                ...
                <button onClick={() => onAddToCart(p)} className="bg-brand-dark text-white p-2 rounded-full hover:bg-brand-gold transition-colors"><ShoppingBag size={18} /></button>
              </div>
            </div>
          </div>
        ))}
...
function BookingPage({ profile, onNavigate }: { profile: any; onNavigate: (p: Page) => void; }) {
  const [bookingData, setBookingData] = useState({ clientName: profile?.name || "", phone: "", treatment: "טיפול פנים קלאסי", date: "", time: "" });
  const [availableTimes, setAvailableTimes] = useState<string[]>([]);
  const [isFetchingSlots, setIsFetchingSlots] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    if (!bookingData.date) { setAvailableTimes([]); return; }
    const loadSlots = async () => {
      setIsFetchingSlots(true);
      try {
        const slots = await fetchAvailableSlots(bookingData.date);
        setAvailableTimes(slots);
      } catch (error) { toast.error("לא ניתן היה לטעון שעות פנויות לתאריך זה"); } 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bookingData.clientName.trim() || !bookingData.phone.trim() || !bookingData.date || !bookingData.time) {
      toast.error("אנא מלאי את כל השדות בטופס"); return;
    }
    const phoneRegex = /^05\d-?\d{7}$/;
    if (!phoneRegex.test(bookingData.phone.trim())) {
      toast.error("אנא הזיני מספר טלפון נייד תקין (למשל: 0501234567)"); return;
    }
    setIsSubmitting(true);
    try {
      await createAppointment({ client_name: bookingData.clientName, phone: bookingData.phone, treatment_type: bookingData.treatment, appointment_date: bookingData.date, appointment_time: bookingData.time });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto px-6 py-12 space-y-12">
      <div className="text-center space-y-4">
        <h2 className="text-4xl serif font-semibold">קביעת תור לקליניקה</h2>
        <p className="text-brand-dark/60">מלאי את פרטייך ובחרי את הטיפול והזמן הנוח לך</p>
      </div>

      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-brand-gold/10 shadow-xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-widest text-brand-gold">שם מלא</label>
            <input type="text" placeholder="ישראלה ישראלי" value={bookingData.clientName} onChange={(e) => setBookingData({ ...bookingData, clientName: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
          </div>
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-widest text-brand-gold">מספר טלפון נייד</label>
            <input type="tel" placeholder="0501234567" value={bookingData.phone} onChange={(e) => setBookingData({ ...bookingData, phone: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
          </div>
        </div>

        <div className="space-y-4">
          <label className="block text-sm font-bold uppercase tracking-widest text-brand-gold">בחרי טיפול</label>
          <select value={bookingData.treatment} onChange={(e) => setBookingData({ ...bookingData, treatment: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <input type="date" min={new Date().toISOString().split("T")[0]} value={bookingData.date} onChange={(e) => setBookingData({ ...bookingData, date: e.target.value, time: "" })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
          </div>
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-widest text-brand-gold">שעה זמינה</label>
            <select value={bookingData.time} onChange={(e) => setBookingData({ ...bookingData, time: e.target.value })} disabled={!bookingData.date || isFetchingSlots} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 disabled:opacity-50">
              <option value="">{isFetchingSlots ? "טוען שעות פנויות..." : "בחרי שעה"}</option>
              {availableTimes.map((time) => <option key={time} value={time}>{time}</option>)}
              {bookingData.date && availableTimes.length === 0 && !isFetchingSlots && <option disabled>אין חלונות זמן פנויים ביום זה</option>}
            </select>
          </div>
        </div>

        <button type="submit" disabled={!bookingData.date || !bookingData.time || isSubmitting} className="w-full bg-brand-gold text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-brand-gold/90 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
          {isSubmitting ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> שומר תור...</> : ("אישור וקביעת התור")}
        </button>
      </form>
    </motion.div>
  );
...
function ProfilePage({ profile, appointments, orders, onCancelAppointment }: { profile: any; appointments: any[]; orders: any[]; onCancelAppointment: (id: string) => void; }) {
  ...
  const handleEditClick = async () => {
    ...
    setEditForm({ firstName: parts[0] || '', lastName: parts.slice(1).join(' ') || '', phone: 'טוען נתונים...' });
    setIsEditing(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const meta = user.user_metadata || {};
        setEditForm({ firstName: meta.first_name || parts[0] || '', lastName: meta.last_name || parts.slice(1).join(' ') || '', phone: meta.phone || '' });
      } else { setEditForm(prev => ({ ...prev, phone: '' })); }
    } catch (err) { setEditForm(prev => ({ ...prev, phone: '' })); }
  };

  const handleSaveProfile = async () => {
    if (!editForm.firstName.trim() || !editForm.lastName.trim() || !editForm.phone.trim()) {
      toast.error('נא למלא את כל השדות', { position: 'top-center' }); return;
    }
    setIsSubmitting(true);
    try {
      const backendUrl = import.meta.env.VITE_DEV_BACKEND_URL || 'http://localhost:3000';
      await fetch(`${backendUrl}/api/profile/update`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: profile.uid, first_name: editForm.firstName, last_name: editForm.lastName, phone: editForm.phone })
      });
      await supabase.auth.updateUser({ data: { first_name: editForm.firstName, last_name: editForm.lastName, phone: editForm.phone } });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto px-6 py-12 space-y-12">
      <div className="bg-white p-8 rounded-3xl border border-brand-gold/10 shadow-lg flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="w-24 h-24 bg-brand-gold/10 rounded-full flex items-center justify-center text-brand-gold"><User size={48} /></div>
          <div className="space-y-2 text-center md:text-right">
            <h2 className="text-3xl serif font-semibold">{profile?.name}</h2>
            <p className="text-brand-dark/60">{profile?.email}</p>
            <div className="inline-block px-3 py-1 bg-brand-gold/10 text-brand-gold text-xs font-bold rounded-full uppercase tracking-widest">לקוחה</div>
          </div>
        </div>
        <button onClick={handleEditClick} className="flex items-center gap-2 bg-brand-beige text-brand-dark px-6 py-3 rounded-xl font-bold hover:bg-brand-gold hover:text-white transition-colors border border-brand-gold/20">
          <Edit3 size={18} /> עריכת פרטים
        </button>
      </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input type="text" value={editForm.firstName} onChange={(e) => setEditForm({ ...editForm, firstName: e.target.value })} className="w-full p-3 rounded-xl border border-brand-gold/20 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 bg-brand-beige/30 text-sm" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-gold">שם משפחה</label>
                  <input type="text" value={editForm.lastName} onChange={(e) => setEditForm({ ...editForm, lastName: e.target.value })} className="w-full p-3 rounded-xl border border-brand-gold/20 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 bg-brand-beige/30 text-sm" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-gold">טלפון</label>
                  <input type="tel" value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} className="w-full p-3 rounded-xl border border-brand-gold/20 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 bg-brand-beige/30 text-sm" dir="ltr" placeholder="05X-XXXXXXX" />
                </div>
              </div>
              <button onClick={handleSaveProfile} disabled={isSubmitting} className="w-full bg-brand-gold text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-brand-gold/90 transition-all shadow-lg disabled:opacity-50 mt-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="space-y-6">
          <h3 className="text-2xl serif font-semibold flex items-center gap-2"><Calendar size={20} className="text-brand-gold" /> התורים שלי</h3>
          <div className="space-y-4">
            {appointments.length === 0 ? <p className="text-brand-dark/40 italic">אין תורים עתידיים</p> : (
              appointments.map((app) => (
                <div key={app.id} className="bg-white p-6 rounded-2xl border border-brand-gold/10 flex justify-between items-center">
                  <div>
                    ...
                    <span className={cn("text-[10px] font-bold uppercase px-2 py-0.5 rounded-full mt-2 inline-block", app.status === "scheduled" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>
                      {app.status === "scheduled" ? "מאושר" : "מבוטל"}
                    </span>
                  </div>
                  {app.status === "scheduled" && (
                    <button onClick={() => onCancelAppointment(app.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors"><Trash2 size={18} /></button>
                  )}
                </div>
              ))
            )}
        ...
        <div className="space-y-6">
          <h3 className="text-2xl serif font-semibold flex items-center gap-2"><ShoppingBag size={20} className="text-brand-gold" /> ההזמנות שלי</h3>
          <div className="space-y-4">
            {orders.length === 0 ? <p className="text-brand-dark/40 italic">אין הזמנות קודמות</p> : (
              orders.map((order) => (
                    ...
                    {order.items && order.items.map((item: any, i: number) => {
                      ...
                      return (
                        <div key={i} className="group relative w-12 h-12 rounded-lg bg-brand-beige overflow-hidden flex-shrink-0 border border-brand-gold/5">
                          <img src={itemImage} alt={item.name} className="w-full h-full object-cover transition-transform group-hover:scale-110" onError={(e) => { (e.target as HTMLImageElement).src = "https://via.placeholder.com/150"; }} />
                          {item.quantity > 1 && <span className="absolute bottom-0 right-0 bg-brand-gold text-white text-[9px] font-bold px-1 rounded-tl">x{item.quantity}</span>}
                        </div>
                      );
                    })}
                  </div>
      ...
      <div className="bg-white rounded-3xl border border-brand-gold/10 shadow-xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right">
              <thead className="bg-brand-beige/50 text-xs uppercase tracking-widest text-brand-gold">
                <tr><th className="p-6">לקוחה</th><th className="p-6">טיפול</th><th className="p-6">תאריך ושעה</th><th className="p-6">סטטוס</th><th className="p-6">פעולות</th></tr>
              </thead>
              <tbody className="divide-y divide-brand-gold/10">
                {appointments.map((app: any) => (
                  <tr key={app.id}>
                    ...
                    <td className="p-6"><span className={cn("text-[10px] font-bold uppercase px-2 py-0.5 rounded-full", app.status === "scheduled" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>{app.status}</span></td>
                    <td className="p-6">
        ...
        {tab === "products" && (
          <div className="p-8 text-center space-y-6">
            ...
            <button className="bg-brand-gold text-white px-8 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition-transform">הוספת מוצר חדש</button>
          </div>
        )}
        ...
        {tab === "blog" && (
          <div className="p-8 space-y-8">
            <div className="bg-brand-beige/30 p-6 rounded-3xl space-y-4">
              ...
              <input type="text" placeholder="כותרת הפוסט" value={newPost.title} onChange={(e) => setNewPost({ ...newPost, title: e.target.value })} className="w-full p-4 rounded-xl border border-brand-gold/20 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
              <textarea placeholder="תוכן הפוסט..." rows={4} value={newPost.content} onChange={(e) => setNewPost({ ...newPost, content: e.target.value })} className="w-full p-4 rounded-xl border border-brand-gold/20 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
              <button onClick={handleAddPost} className="bg-brand-gold text-white px-8 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition-transform">פרסמי פוסט</button>
            </div>
            <div className="space-y-4">
              ...
              {blogPosts.map((post: any) => (
                <div key={post.id} className="bg-white p-6 rounded-2xl border border-brand-gold/10 flex justify-between items-center">
                  ...
                  <button onClick={() => onDeleteBlogPost(post.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors"><Trash2 size={18} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === "reviews" && (
          <div className="overflow-x-auto">
            <table className="w-full text-right">
              <thead className="bg-brand-beige/50 text-xs uppercase tracking-widest text-brand-gold">
                <tr><th className="p-6">לקוחה</th><th className="p-6">טיפול</th><th className="p-6">דירוג</th><th className="p-6">סטטוס</th><th className="p-6">פעולות</th></tr>
              </thead>
              <tbody className="divide-y divide-brand-gold/10">
                {reviews.map((review: any) => (
                  <tr key={review.id}>
                    ...
                    <td className="p-6"><span className={cn("text-[10px] font-bold uppercase px-2 py-0.5 rounded-full", review.status === "approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700")}>{review.status === "approved" ? "מאושר" : "ממתין"}</span></td>
                    <td className="p-6 flex gap-2">
                      {review.status === "pending" && <button onClick={() => onUpdateReview(review.id, "approved")} className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100"><CheckCircle2 size={16} /></button>}
          ...
          <div className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <input placeholder="שם הטיפול" value={newTreatment.name} onChange={(e) => setNewTreatment({ ...newTreatment, name: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4" />
                <textarea placeholder="תיאור" value={newTreatment.description} onChange={(e) => setNewTreatment({ ...newTreatment, description: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4 h-32" />
                <input placeholder="מחיר (₪)" value={newTreatment.price} onChange={(e) => setNewTreatment({ ...newTreatment, price: e.target.value })} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-xl p-4" />
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto px-6 py-12 space-y-12">
      ...
      <div className="space-y-12">
        {posts.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-brand-gold/10 italic text-brand-dark/40">בקרוב יעלו פוסטים חדשים...</div>
        ) : (
          posts.map((post) => (
            <article key={post.id} className="bg-white p-8 md:p-12 rounded-[40px] border border-brand-gold/10 shadow-xl space-y-6 hover:shadow-2xl transition-shadow">
              ...
              <div className="prose prose-brand max-w-none text-brand-dark/70 leading-relaxed whitespace-pre-wrap">{post.content}</div>
              <div className="pt-6 border-t border-brand-gold/10 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-brand-gold/10 flex items-center justify-center text-brand-gold"><User size={20} /></div>
...
function ConsultationPage({ onOpenAssistant }: { onOpenAssistant: () => void; }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.message) { toast.error("אנא מלאי את כל השדות"); return; }
    setIsSubmitting(true);
    try {
      await addSupabaseData(formData.name, formData.message);
      toast.success("ההודעה נשמרה בהצלחה ב-Supabase דרך הבקאנד!");
      setFormData({ name: "", message: "" });
    } catch (error) { toast.error("חלה שגיאה בשמירת הנתונים. וודאי שהבקאנד פועל."); } 
    finally { setIsSubmitting(false); }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div className="space-y-6">
          ...
          <p className="text-brand-dark/70 leading-relaxed">
            האסיסטנטית הווירטואלית שלנו פותחה במיוחד כדי לעזור לך להבין את סוג
            העור שלך ולהמליץ על המוצרים והטיפולים המדויקים ביותר עבורך. היא
            זמינה 24/7 לכל שאלה שיש לך בנושא טיפוח ואסתטיקה.
          </p>
          ...
          <button onClick={onOpenAssistant} className="bg-brand-dark text-white px-10 py-4 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-brand-gold transition-all shadow-xl flex items-center gap-3">
      ...
      <div className="bg-white rounded-[40px] p-8 md:p-16 border border-brand-gold/10 shadow-2xl space-y-12">
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="ישראל ישראלי" className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-2xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold uppercase tracking-widest text-brand-gold">הודעה / תוכן</label>
            <textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} placeholder="ספרי לנו במה נוכל לעזור..." rows={4} className="w-full bg-brand-beige/30 border border-brand-gold/20 rounded-2xl p-4 focus:outline-none focus:ring-2 focus:ring-brand-gold/30" />
          </div>
          <button type="submit" disabled={isSubmitting} className="w-full bg-brand-gold text-white py-4 rounded-2xl font-bold uppercase tracking-widest hover:bg-brand-gold/90 transition-all shadow-lg disabled:opacity-50">
